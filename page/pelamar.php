                    ...
                    <div class="table-responsive">
                        <table class="table card-table table-vcenter text-nowrap datatable">
                            ...
                            <tbody>
                                <?php
                                $no = 1;
                                $query = $koneksi->query("select * from pelamar");
                                while ($hasil = $query->fetch_assoc()) {
                                ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td><?= $hasil['kode_pelamar'] ?></td>
                                        <td><?= $hasil['nama_pelamar'] ?></td>
                                        <td><?= $hasil['alamat'] ?></td>
                                        <td><?= $hasil['telp_pelamar'] ?></td>
                                        <td><a onclick="ubah('<?= $hasil['kode_pelamar'] ?>')" class="btn btn-primary btn-sm">Edit</a> | <a onclick="return confirm('Apakah kamu yakin akan menghapus?')" href="posts/pelamar-proses.php?tipe=hapus&kode=<?= $hasil['kode_pelamar'] ?>" class="btn btn-danger btn-sm">Hapus</a></td>
                                    </tr>
                                <?php } ?>
                            </tbody>
                        </table>
